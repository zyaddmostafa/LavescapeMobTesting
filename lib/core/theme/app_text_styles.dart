import 'package:flutter/material.dart';
import 'app_color.dart';

class AppTextStyles {
  const AppTextStyles._();

  //========= Bold Text Styles =========//
  static const TextStyle font18BoldBlack = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w700,
    color: AppColor.black,
  );
  //========= Semibold Text Styles =========//
  static const TextStyle font20SemiBoldWhite = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
    color: Colors.white,
  );
  static const TextStyle font18SemiBoldBlack = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
    color: Colors.black,
  );

  static const TextStyle font15SemiBoldWhite = TextStyle(
    fontSize: 15,
    fontWeight: FontWeight.w600,
    color: Colors.white,
  );

  static const TextStyle font15SemiBoldPurple = TextStyle(
    fontSize: 15,
    fontWeight: FontWeight.w600,
    color: AppColor.primary,
  );

  static const TextStyle font15SemiBoldLabelColor = TextStyle(
    fontSize: 15,
    fontWeight: FontWeight.w600,
    color: AppColor.labelColor,
  );
  static const TextStyle font14SemiBoldLabelColor = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
    color: AppColor.labelColor,
  );

  static const TextStyle font14SemiBoldBlack = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
    color: Colors.black,
  );
  static const TextStyle font14SemiBoldPurple = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
    color: AppColor.primary,
  );
  //========= Medium Text Styles =========//

  static const TextStyle font16MediumBlack = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
    color: Colors.black,
  );

  static const TextStyle font15MediumLabelColor = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
    color: AppColor.labelColor,
  );
  static const TextStyle font15MediumGrey = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
    color: AppColor.secondary,
  );

  static const TextStyle font14MediumLabelColor = TextStyle(
    fontSize: 14,
    fontWeight: FontWeight.w600,
    color: AppColor.labelColor,
  );
  static const TextStyle font12MediumGrey = TextStyle(
    fontSize: 12,
    fontWeight: FontWeight.w600,
    color: AppColor.secondary,
  );

  static const TextStyle font10MediumGrey = TextStyle(
    fontSize: 10,
    fontWeight: FontWeight.w600,
    color: AppColor.secondary,
  );

  static const TextStyle font10MediumBlack = TextStyle(
    fontSize: 10,
    fontWeight: FontWeight.w600,
    color: AppColor.black,
  );
  static const TextStyle font10MediumPurple = TextStyle(
    fontSize: 10,
    fontWeight: FontWeight.w600,
    color: AppColor.primary,
  );
  //========= Regular Text Styles =========//

  static const TextStyle font16RegularWhite = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w500,
    color: Colors.white,
  );

  static const TextStyle font14RegularGrey = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w500,
    color: AppColor.secondary,
  );
  static const TextStyle font14RegularBlack = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w500,
    color: AppColor.black,
  );
  static const TextStyle font12RegularGrey = TextStyle(
    fontSize: 12,
    fontWeight: FontWeight.w500,
    color: AppColor.secondary,
  );

  static const TextStyle font12RegularPurple = TextStyle(
    fontSize: 12,
    fontWeight: FontWeight.w500,
    color: AppColor.primary,
  );
}
